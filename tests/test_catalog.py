...
from maam.config.models import UserConfig, AssetKind, AssetManifest
...
def test_precedence(mock_home, monkeypatch, tmp_path):
    ...
    save_model(remote_asset / "ASSET.yaml", AssetManifest(
    ...
    save_model(local_path / "ASSET.yaml", AssetManifest(
...
def test_remove_asset(catalog_manager, tmp_path):
    ...
    save_model(asset_src / "ASSET.yaml", AssetManifest(
        name="to-remove", kind="agent", version="1.0", description="rem"
    ))
